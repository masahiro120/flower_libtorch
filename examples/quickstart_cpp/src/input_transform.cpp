...
InputTransformImpl::InputTransformImpl(int K){
    ...
    bn_2 = register_module("bn_2", torch::nn::BatchNorm1d(128));
    bn_3 = register_module("bn_3", torch::nn::BatchNorm1d(1024));
...
at::Tensor InputTransformImpl::forward(torch::Tensor x){
    ...
    x = fc_1->forward(x);
    x = fc_2->forward(x);
    auto transform = torch::zeros({256,K*K}, torch::requires_grad()).to(torch::kCPU);
